import type { LegacyProjectStatus as ProjectStatus } from '@/types/database'
...
export function getStatusConfig(status: ProjectStatus): StatusConfig {
  switch (status) {
    ...
    case 'processing-feasibility':
      return { label: 'Analyzing', variant: 'secondary', className: 'bg-indigo-100 text-indigo-800 border-indigo-200' }
